...
if [ "${OS_ID_LIKE}" == "rhel" ]; then
	...
	RUNTIME_DEPS+=(ninja-build perl-IPC-Cmd zip ccache wget lz4-devel czmq-devel openssl-devel sqlite-devel libspatialite-devel libcurl-devel luajit-devel gdal-devel)
elif [ "${OS_ID_LIKE}" == "debian" ]; then
